...
let showAllMode = false; // false - режим "Ещё +25", true - показаны все записи
...
let currentType = 'appeal'; // 'consultation', 'duty', 'appeal'
let loadedCount = 25;
...
// ========== ЗАГРУЗКА ДАННЫХ (с поддержкой "Ещё") ==========
async function loadHistoryData() {
    ...
    try {
        let tableName;
        if (currentType === 'consultation') tableName = 'Consultation_scenario';
        else if (currentType === 'duty') tableName = 'duty_room';
        else tableName = 'appeals';

        // Загружаем на 1 запись больше, чтобы понять, есть ли ещё
        const fetchCount = loadedCount + 1;

        let query = sb.from(tableName).select('*', { count: 'exact' }).order('created_at', { ascending: false }).limit(fetchCount);
        ...
        if (records.length > loadedCount) {
            hasMore = true;
            records.pop(); // Убираем лишнюю запись
        } else {
            hasMore = false;
        }
...
async function loadAllData() {
    ...
    try {
        ...
        console.log('Загрузка ВСЕХ записей из таблицы:', tableName);

        const response = await sb.from(tableName).select('*', { count: 'exact' }).order('created_at', { ascending: false });

        totalAvailable = response.count || 0;
        const records = response.data || [];

        console.log('Получено записей:', records.length, 'Всего в БД:', totalAvailable);
...
function updateButtonsForMode() {
    ...
    if (showAllMode) {
        ...
        if (showAllBtn) showAllBtn.textContent = '📋 Показано всё';
        if (showAllBtn) showAllBtn.style.background = '#28a745';
        if (showAllBtn) showAllBtn.disabled = true;
    } else {
        if (showAllBtn) showAllBtn.textContent = '📋 Показать все';
        if (showAllBtn) showAllBtn.style.background = '#17a2b8';
...
function updateRecordsCounter() {
    const counterSpan = document.getElementById('recordsCount');
    const loadMoreBtn = document.getElementById('loadMoreBtn');
    const showAllBtn = document.getElementById('showAllBtn');

    if (counterSpan) {
        counterSpan.textContent = `Показано: ${historyRecords.length} из ${totalAvailable}`;
    }
...
function applyHistoryFilters() {
    ...
    if (historyFilters.dateFrom) {
        filtered = filtered.filter(r => r.displayDate >= historyFilters.dateFrom);
    }
    if (historyFilters.dateTo) {
        filtered = filtered.filter(r => r.displayDate <= historyFilters.dateTo);
    }
...
async function loadHistoryData() {
    ...
    // Если сейчас режим "Показать все", сбрасываем его при новом типе таблицы
    if (showAllMode) {
        showAllMode = false;
        loadedCount = 25;
    }
    ...
    try {
        ...
        console.log('Загрузка из таблицы:', tableName, 'loadedCount:', loadedCount);
...
function renderHistoryTable() {
    ...
    tbody.innerHTML = filteredHistoryRecords.map((record, index) => {
        ...
        if (record.source === 'consultation') {
            sourceClass = 'source-consultation';
            sourceText = '📋 Консультация';
            let html = `🔗 <a href="${record.link}" target="_blank">${record.link.length > 60 ? record.link.substring(0, 60) + '...' : record.link}</a>`;
            if (record.comment && record.comment.trim() !== '') {
                html += `<br>💬 ${record.comment}`;
            }
            dataHtml = html;
        }
        // ========== ДЕЖУРКА ==========
            ...
            const periodText = record.period_from ? `${formatDate(record.period_from)} — ${formatDate(record.period_to)}` : record.period;
            let html = `📅 Период: ${periodText}<br>🔢 Количество: ${record.quantity}`;

            if (record.measures?.length > 0) {
                let measuresHtml = '<ul style="margin: 5px 0 0 15px;">' + record.measures.map(m => {
                    switch(m.type) {
                        case 'new_type': return `<li>Новый вид: ${m.value}</li>`;
                        case 'new_solution': return `<li>Новое решение: ${m.value}</li>`;
                        case 'task': return `<li><a href="${m.value}" target="_blank">Задача</a></li>`;
            ...
            if (record.measures?.length > 0) {
                let measuresHtml = '<ul style="margin: 5px 0 0 15px;">' + record.measures.map(m => {
                    switch(m.type) {
                        ...
                        case 'task': return `<li><a href="${m.value}" target="_blank">Задача</a></li>`;
                        case 'error': return `<li><a href="${m.value}" target="_blank">Ошибка</a></li>`;
                        case 'help': return `<li><a href="${m.value}" target="_blank">Задача на создание/изменение документации на help</a></li>`;
                        default: return '';
                    }
                }).join('') + '</ul>';
                html += `<br>📋 Меры: ${measuresHtml}`;
...
// ========== РЕДАКТИРОВАНИЕ ОБРАЩЕНИЯ (МОДАЛЬНОЕ ОКНО) ==========
    ...
    if (data.measures && data.measures.length > 0) {
        measuresHtml = data.measures.map((m, idx) => {
            ...
            switch(m.type) {
                ...
                case 'task': typeClass = 'task'; title = '🚀 Задача'; break;
                case 'error': typeClass = 'error'; title = '⚠️ Ошибка'; break;
                case 'help': typeClass = 'help'; title = '📚 Задача на создание/изменение документации на help'; break;
                default: return '';
            }
            return `
                <div class="measure-card measure-${typeClass}" style="background: #f8f9fa; padding: 12px; border-radius: 8px; margin-bottom: 10px; border: 1px solid #e0e0e0;">
...
function openStatsReport() {
    ...
    // Определяем, сколько типов таблиц выбрано
    const selectedTypes = [];
    if (consultationRecords.length > 0) selectedTypes.push('consultation');
    if (dutyRecords.length > 0) selectedTypes.push('duty');
    if (appealRecords.length > 0) selectedTypes.push('appeal');

    const isSingleType = selectedTypes.length === 1;
    const singleType = selectedTypes[0];

    // Подсчёт статистики ДЛЯ ДЕЖУРКИ
    let dutyStats = {
        totalQuantity: 0,
        newTypes: [],
        newSolutions: [],
    ...
    // Подсчёт статистики ДЛЯ ОБРАЩЕНИЙ
    let appealStats = {
        newTypes: [],
        newSolutions: [],
        tasks: [],
    ...
    let statsHtml = `
        <div style="background: white; padding: 30px; border-radius: 20px; width: ${isSingleType ? '900px' : '1400px'}; max-width: 95%; max-height: 85vh; overflow-y: auto; position: relative;">
    ...
    // ========== ЕСЛИ ВЫБРАН ТОЛЬКО ОДИН ТИП ТАБЛИЦЫ ==========
    if (isSingleType) {
        if (singleType === 'consultation') {
            statsHtml += `
            ...
            `;
        } else if (singleType === 'duty') {
            statsHtml += `
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
                    <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold;">${dutyStats.totalQuantity}</div>
                        <div style="font-size: 13px;">Всего обращений</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #17a2b8;">${dutyStats.newTypes.length}</div>
                        <div style="font-size: 13px;">Новых видов</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #28a745;">${dutyStats.newSolutions.length}</div>
                        <div style="font-size: 13px;">Новых решений</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #ffc107;">${dutyStats.tasks.length}</div>
                        <div style="font-size: 13px;">Задач</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #dc3545;">${dutyStats.errors.length}</div>
                        <div style="font-size: 13px;">Ошибок</div>
                    </div>
                </div>
                <div style="margin-bottom: 25px; border: 1px solid #e0e0e0; border-radius: 16px; overflow: hidden;">
                    <div onclick="toggleSection(this)" style="cursor: pointer; background: #f8f9fa; padding: 16px 20px; font-weight: 700; font-size: 18px; display: flex; justify-content: space-between; align-items: center;">
            ...
            `;
        } else if (singleType === 'appeal') {
            statsHtml += `
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
                    <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold;">${appealRecords.length}</div>
                        <div style="font-size: 13px;">Обращений</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #ffc107;">${appealStats.tasks.length}</div>
                        <div style="font-size: 13px;">Задач</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #dc3545;">${appealStats.errors.length}</div>
                        <div style="font-size: 13px;">Ошибок</div>
                    </div>
                    <div style="background: #f0f4f8; padding: 20px; border-radius: 16px; text-align: center;">
                        <div style="font-size: 32px; font-weight: bold; color: #6f42c1;">${appealStats.helps.length}</div>
                        <div style="font-size: 13px;">HELP</div>
                    </div>
                </div>
                <div style="margin-bottom: 25px; border: 1px solid #e0e0e0; border-radius: 16px; overflow: hidden;">
                    <div onclick="toggleSection(this)" style="cursor: pointer; background: #f8f9fa; padding: 16px 20px; font-weight: 700; font-size: 18px; display: flex; justify-content: space-between; align-items: center;">
        ...
        // ========== ЕСЛИ ВЫБРАНО НЕСКОЛЬКО ТИПОВ ТАБЛИЦ ==========
        statsHtml += `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Переключатели типа в истории
    document.querySelectorAll('.type-switch-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const type = btn.getAttribute('data-type');
            console.log('Нажата кнопка типа:', type);
            if (type) switchHistoryType(type);
        });
    });

    // Кнопка "Ещё"
    document.getElementById('loadMoreBtn')?.addEventListener('click', loadMoreRecords);

    // При переключении на вкладку истории загружаем выбранный тип
    if (document.getElementById('tabHistory').style.display === 'block') {
        loadHistoryData();
    }
...
document.getElementById('addAppealBtn')?.addEventListener('click', async () => {
    ...
    if (showAllBtn) {
        showAllBtn.addEventListener('click', () => {
            console.log('Кнопка "Показать все" нажата');
            showAllMode = true;
            loadAllData();
        });
    }

    // Кнопка "Ещё +25" (оставляем как есть)
    const loadMoreBtn = document.getElementById('loadMoreBtn');
    if (loadMoreBtn) {
